...
//
// БЫЛО (кратко) WDD:3/WCC:3/WBB:20 — цена протокола B→1→2 (сессия
// 2026-09-25), пользователь перепроверил методологию и откатил (см.
// scripts/balance/protocol-v2.md, known-issue #35 переоткрыт): B давал
// ОДИНАКОВЫЕ статы обеим сторонам (симметрия → коэффициенты около нуля по
// построению, не потому что статы не важны), цена выводилась из
// коэффициентов логистической регрессии на коллинеарных переменных (при
// фиксированном бюджете/числе юнитов сумма разниц состава фиксирована) —
// 3/3/20 был артефактом обрезки `max(coeff,0.05)`: WDD и WCC оба ниже
// порога → одинаковая цена, хотя WCC заведомо сильнее WDD. Ни один метод
// не проверялся на заранее известном ответе (нет позитивного контроля).
// Откат на исходную 5/8/12 — протокол v2 (асимметричные эксперименты,
// точка безубыточности вместо регрессии, позитивные контроли K1/K2)
// перезапускается с нуля, см. scripts/balance/protocol-v2.md.
export const SPAWN_COSTS = { WDD: 5, WCC: 8, WBB: 12 };
