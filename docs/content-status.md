...
| Sail | Перемещение по water и deep с уменьшением дальности хода | ✅ | `moveTerrain`, `moRange`. **Описание неточное**: реально даёт только `surf`+`water`, НЕ `deep` — код и калибровка баланса важнее текста, не правили |
| Navy | Улучшенное перемещение по deep и +1 к дальности атаки | ✅ | `moveTerrain`, `moRange`. **Описание неточное**: код даёт `+1 moRange` (дальность ХОДА), не `atRange` — не правили, числа откалиброваны под текущее поведение |
| Draft (сессия 9) | Открывает deep для передвижения, без изменения дальности хода | ✅ | `moveTerrain` — `getAvailableHexes()`. Для юнитов с deep-доступом без Navy-компенсации Sail-штрафа (WDD, WCA) |
| Submerge (сессия 9) | Двигается по water и deep, не по surf. Тот же штраф хода, что у Sail | ✅ | `moveTerrain`, `moRange`. Замена Sail для подлодок (WSB) — та же -1 к moRange, но water+deep вместо surf+water |
...
12 из ~37 модулей/флагов (Dual, Sail, Navy, Draft, Submerge, Air, Charge,
Flee, Percy, Splash, `noCounter`, плюс частично Corrupt/Surge через
отдельный механизм без итогового эффекта) реально влияют на геймплей.
Остальные ~25 — описаны в реестре, выставляют флаг на юните, но ни один
файл игровой логики этот флаг не проверяет.
