...
- `handleCanvasClick()` (`ui/events.js`) — клик игрока по гексу с невидимым
  вражеским юнитом всё ещё технически находит его в `state.units` (юнит
  просто не нарисован, но не защищён от угадывания координат вслепую).
  Минорная дыра, не критично для v1.
